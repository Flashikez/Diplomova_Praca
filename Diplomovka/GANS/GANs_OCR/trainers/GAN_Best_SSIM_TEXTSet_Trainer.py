...
# Trénuje GAN, výsledok bude model s najvacsim SSIM indexom
class GAN_Best_SSIM_TEXTSet_Trainer():
	...
	def start(self,gen_save_path,discr_save_path,generator_save_name = "generator",discrim_save_name="discriminator",epochs=12,save_best_classified_model=True,plot_batch_after_epoch=True,plot_batch_size=8,plot_path='plot/',log_losses=True,log_metrics=True, log_path="log/",plot_title='Tréning GANu'):
		...
		for epoch in range(1,epochs+1):
			...
			if metric_value > bestMetric_Value and save_best_classified_model is True:
				print(f'Found best metric value epoch {epoch}, accuracy {metric_value}')
				print('Saving models')
				self.save_models(self.generator, self.discriminator, gen_save_path + generator_save_name,
								 discr_save_path + discrim_save_name)
				bestMetric_Value = metric_value
