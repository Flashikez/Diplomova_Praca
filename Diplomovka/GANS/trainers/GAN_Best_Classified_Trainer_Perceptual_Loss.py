...
# Tréner GANu metódou najlepšej klasifikácie na klasifikátore classifier, s percepčnou chybovou funkciou generátora
class GAN_perceptual_loss_trainer():
	...
	def _train_step(self,real_images,gen_input):
		with tf.GradientTape() as generator_tape , tf.GradientTape() as discrim_tape:
			...
			sr_features = self.classifier.hidden_layer_output(gen_imgs)
			hr_features = self.classifier.hidden_layer_output(real_images)
			content_loss = self.mse(hr_features, sr_features)
			percept_loss =   ((10 ** -2) * content_loss) + gen_loss

		# Úprava váh generátora a diskriminátora
		self.generator.calc_apply_gradients(generator_tape,percept_loss)
		self.discriminator.calc_apply_gradients(discrim_tape,discrim_loss)
		return percept_loss,discrim_loss
