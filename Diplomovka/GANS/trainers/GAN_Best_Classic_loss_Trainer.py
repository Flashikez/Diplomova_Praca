...
# Tréner GANu na základe hodnoty chybovej funkcie generátora na testovacej sade.
# Toto v práci nevyužívame
class GAN_Best_classic_loss_trainer():
	...
	def start(self,gen_save_path,discr_save_path,generator_save_name = "generator",discrim_save_name="discriminator",epochs=12,save_best_loss_model=True,plot_batch_after_epoch=True,plot_batch_size=8,plot_path='plot/',log_losses=True,log_metrics=True,log_loss = True,log_acc=True, log_path="log/",plot_title='Tréning GANu'):
		...
		if log_loss:
			...
			loss = self.generator_loss_on_datasets(self.generator,self.discriminator,self.test_dataset_gen_inputs,self.test_dataset_real)
			self.log_gen_loss(0,loss,log_path)
		if log_acc:
			print('Calculating accuracy of generated test set on classifier before training')
		...
		for epoch in range(1,epochs+1):
			...
			print('Calculating total generator loss of generated test dataset from generator')
			loss = self.generator_loss_on_datasets(self.generator,self.discriminator,self.test_dataset_gen_inputs,self.test_dataset_real)

			if log_acc:
				print('Calculating accuracy of generated test')
				accuracy = self.generator_accuracy_test(self.generator, self.classifier, self.accuracy_dataset)
				self.log_accuracy(epoch, accuracy, log_path)

			if log_loss:
				self.log_gen_loss(epoch, loss, log_path)

			print(f'accuracy: {loss}')
			if loss < bestLoss and save_best_loss_model is True:
				print(f'Found best loss epoch {epoch}, accuracy {loss}')
				print('Saving models')
	...
	def log_gen_loss(self,epoch,loss,log_path):
		if epoch == 0:
			class_utils.append_to_file(f"{log_path}trainlog_generator_loss.txt", "epoch:loss\n")

		class_utils.append_to_file(f"{log_path}trainlog_generator_loss.txt", f"{epoch}:{loss}\n")
	...
	def generator_loss_on_datasets(self, generator,discriminator, test_dataset_gen_inputs, test_dataset_real):
		total_loss = 0
		samples = 0
		for test_batch, orig_batch in zip(test_dataset_gen_inputs, test_dataset_real):
			batch_size = test_batch.shape[0]
			generated = generator.output(test_batch,is_training=False)
			disc_output = discriminator.output(generated,is_training=False)
			total_loss += generator.loss(disc_output)
			samples += batch_size
